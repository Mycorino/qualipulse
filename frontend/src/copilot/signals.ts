...
/**
 * Diff the studies list against the stored baseline and emit a nudge for any
 * study that gained a report since last seen. Returns all active workspace
 * nudges.
 */
export function detectWorkspaceNudges(studies: StudyReportInput[]): Nudge[] {
  ...
  return store.nudges.filter(
    (n) => n.event === "study_report_ready" && studyIds.has(n.scopeId),
  );
}
