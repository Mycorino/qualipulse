...
|---|---|---|---|
| **6: Schema + minimal API** | Alembic 0024 (`studies`, `surveys`, `survey_questions`, `survey_links`, `survey_responses`, `survey_response_answers`, `study_participants`, `consent_acknowledgments`). FastAPI routes: `POST/GET/PATCH/DELETE /surveys`, `POST/GET /surveys/{id}/questions`, `POST /surveys/{id}/responses` (public). 8–10 backend tests. Postman collection. | Frontend (none yet). AI question linting. Quotas. Bridge. | Nothing. |
| **7: Survey builder UI (wraps existing components)** | `/surveys/{id}/edit` page wiring `SurveyQuestionEditor`, `QuestionTypeCard`. Drag-reorder. Save autosaves to API. `/surveys/{id}/preview` renders the public form. 5 component tests. | Live preview of dashboard. Branching. Templates. AI assist. | Sprint 6. |
| **8: Public response page + basic dashboard (the wedge end-to-end)** | `/r/{survey_token}` public response page (mobile-first, no auth). Submission writes `survey_responses` + answers. `/surveys/{id}/dashboard` renders `DashboardShell`, `StatHero`, `ChartCard` for each question, `MethodologyBox`. Wilson CIs. <n=30 rule enforced. | Cross-tabs. Compare segments. AI clustering. Bridge. | Sprint 7. |
| **9: Screener Bridge — the wedge core** | `ScreenerBridge` component wired to filter respondents by answer values. "Invite 89 to interview" creates a new `interview_link` per filtered respondent and emails them via existing email flow. Reuses existing `consume_interview` ledger event when each completes. Identity carried via `study_participant_id`. | Cross-tabs. Validation surveys. Mixed-methods report. | Sprint 8 + existing interview flow. |
| **10: Cross-tabs + segment compare** | `CrossTabTable`, `Segment2x2`, `CompareChips` wired. "Show themes that over-index on respondents who scored ≤6 on Q3" query. Backend: `GET /studies/{id}/cross-tab?row_q=…&col_q=…`. Chi-square + Cramér's V where n permits; refuses with explainer otherwise. | AI synthesis. PrioritizationMatrix. | Sprint 8. |
| **11: Mixed-methods report (Claude synthesis)** | `ExecutiveSummary`, `FindingCard`, `ReportScroll`, `PrioritizationMatrix`, `VerbatimCard` wired. New `study_analyses` table (parallel to `project_analyses`). `POST /studies/{id}/analyses` triggers Claude with survey aggregates + interview transcripts. AI sees aggregates, NOT row-level open-text (cost). Print stylesheet exercised. | Real-time updates. Annotation. Multi-version like ProjectAnalysis has. | Sprints 8, 9, 10. |
| **12: Pricing/billing wiring** | Seed new `PlanEntitlement` rows. `/billing/plans` and frontend pricing page surface survey quotas. Quota-exceeded path on response page. Admin override. | Survey overage billing (killed). Per-response credits (killed). | Sprints 6, 8. |
| **13: AI question linting (`MethodologyBox` advisor)** | `POST /surveys/{id}/questions/lint` returns flags (double-barreled, leading, unbalanced scale, agree/disagree bias). Renders inline in `SurveyQuestionEditor`. Lint is **non-blocking advisory only.** | Sample-size recommendations from AI (killed — methodologist's hard line). Statistical claim review by AI (killed). | Sprint 7. |
| **14: Validation micro-survey + post-interview hook** | Post-interview screen offers "30-second validation survey" linked to the same Study. Joins on `study_participant_id`. Surfaces in the mixed-methods report as a third panel. | Triggered/branching validation. NPS-only auto-trigger. | Sprints 8, 11. |
...
- Sprint 11 (mixed-methods report) is the strategic moat but it's expensive and depends on cross-tabs. Putting it at 11 instead of earlier is correct.
